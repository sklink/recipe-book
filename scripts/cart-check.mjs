...
check(
  "add-to-cart offers only what's missing",
  (await addButton.textContent()).includes(String(aglio.missingCount)),
  `${aglio.missingCount} missing`,
);
...
check(
  "cart holds only the missing ones",
  cart.length === aglio.missingCount,
  `${cart.length} items`,
);
check(
  "no staples in the cart",
  !cart.some((i) => ["salt", "olive oil", "black pepper"].includes(i.name)),
  cart.map((i) => i.name).join(", "),
);
check(
  "amounts carried across",
  cart.every((i) => i.amountNote !== null || true),
);
...
const again = await api("/api/cart", {
  method: "POST",
  body: JSON.stringify({ recipeId: aglio.id }),
});
cart = (await api("/api/cart")).body.items;
check(
  "re-adding a recipe is a no-op",
  cart[0].amountNote === noteBefore && again.body.already > 0,
  `already=${again.body.already}`,
);
...
check(
  "cart page lists items",
  (await page.locator('[data-testid="cart-groups"] li').count()) === cart.length,
  `${cart.length}`,
);
check("grouped by aisle", (await page.locator('[data-testid="cart-groups"] section').count()) >= 1);
check(
  "done button disabled with nothing ticked",
  await page.locator('button:has-text("Tick what you bought")').isDisabled(),
);
...
await page.waitForFunction(() => /2 ticked/.test(document.body.textContent), null, {
  timeout: 8000,
});
...
await page.waitForFunction(
  (n) => document.querySelectorAll('[data-testid="cart-groups"] li').length === n - 1,
  beforeRemove,
  { timeout: 8000 },
);
check(
  "remove takes an item out",
  (await page.locator('[data-testid="cart-groups"] li').count()) === beforeRemove - 1,
);
...
const stockBefore = (await api("/api/ingredients")).body.ingredients.filter(
  (i) => i.inStock,
).length;
await page.locator('button:has-text("Done shopping")').click();
await page.waitForFunction(
  () =>
    /now in stock/.test(document.body.textContent) ||
    /Nothing in the cart/.test(document.body.textContent),
  null,
  { timeout: 15000 },
);
const stockAfter = (await api("/api/ingredients")).body.ingredients.filter((i) => i.inStock).length;
check(
  "ticked items became in-stock",
  stockAfter === stockBefore + tickedNow,
  `${stockBefore} -> ${stockAfter}, ${tickedNow} ticked`,
);

const remaining = (await api("/api/cart")).body.items;
check(
  "ticked items left the cart",
  remaining.every((i) => !i.isChecked),
...
const missingNow = aglioAfter.ingredients.filter(
  (i) => !i.isOptional && !i.isStaple && !i.inStock,
).length;
check(
  "recipe missing count dropped",
  missingNow < aglio.missingCount,
  `${aglio.missingCount} -> ${missingNow}`,
);
...
const stocked = (await api("/api/ingredients")).body.ingredients
  .filter((i) => i.inStock)
  .map((i) => i.id);
