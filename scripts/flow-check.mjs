...
check("sidebar hidden during the flow", await page.locator('aside[aria-label="Ingredients"]').count() === 0);
...
check("step 2 offers 3 time buckets", await page.locator('a[href^="/flow?meal=dinner&time="]').count() === 3);
...
check("step 3 offers cookbook and generate", await page.locator('a[href*="/recipes?"], a[href*="/generate?"]').count() === 2);
...
check("cookbook branch lands on filtered results", cards === 3, `${cards} cards (dinner + commitment)`);
check("filters shown as chips", await page.locator('a[aria-label^="Remove filter"]').count() === 2);
...
await page.waitForFunction(() => document.querySelectorAll('[data-testid="recipe-grid"] article').length > 3, null, { timeout: 15000 });
...
check("remaining chip persists", await page.locator('a[aria-label^="Remove filter"]').count() === 1);

// --- clear all
await page.goto("http://localhost:3000/recipes?mealType=dinner&timeBucket=quick", { waitUntil: "networkidle" });
...
await page.waitForFunction(() => document.querySelectorAll('[data-testid="recipe-grid"] article').length === 23, null, { timeout: 15000 });
check("clear all restores the full cookbook", true, "23 cards");

// --- generate branch carries context
await page.goto("http://localhost:3000/flow?meal=breakfast&time=quick", { waitUntil: "networkidle" });
await page.locator('a[href*="/generate?"]').click();
await page.waitForURL((u) => u.pathname === "/generate");
check("generate branch carries the flow's answers", (await page.textContent('[data-testid="generate-context"]')).includes("Breakfast"));

// --- deep link and garbage handling
await page.goto("http://localhost:3000/flow?meal=lunch&time=average", { waitUntil: "networkidle" });
check("deep link resumes at step 3", (await page.textContent("body")).includes("Something you know"));
await page.goto("http://localhost:3000/flow?meal=brunch&time=eternal", { waitUntil: "networkidle" });
check("garbage params fall back to step 1", (await page.textContent("body")).includes("What meal is this"));
...
const small = await mp.evaluate(() => [...document.querySelectorAll("a")].filter((e) => { const r = e.getBoundingClientRect(); return r.height > 0 && r.height < 44; }).length);
